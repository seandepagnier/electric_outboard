...
// modified by Sean D'Epagnier for hdt5m pulleys for boat motor and to print the tooth count on the pulley
...
draw_text = 1; // draw tooth count on pulley
...
retainer_ht = 3.5;	// height of retainer flange over pulley, standard = 1.5
idler = 1;			// Belt retainer below teeth, 0 = No, 1 = Yes
idler_ht = 4;		// height of idler flange over pulley, standard = 1.5

pulley_t_ht = 15.5;	// length of toothed part of pulley, standard = 12
pulley_b_ht = 4;		// pulley base height, standard = 8. Set to same as idler_ht if you want an idler but no pulley.
...
GT2_5mm_pulley_dia = tooth_spacing (4.9,.5715);
       ...
       difference() {
        pulley ( "GT2 5mm" , GT2_5mm_pulley_dia , 1.969 , 3.952 );
           ...
           //cylinder(r=80, h=30);
            ...
            for(i=[1:spokes])
                rotate(i*360/spokes) {
            translate([GT2_5mm_pulley_dia/3, 0, 0])
                 cylinder(r=GT2_5mm_pulley_dia/8, h=100, center=true);
                 rotate(180/spokes) {
            translate([GT2_5mm_pulley_dia/6, 0, 0])
                 cylinder(r=GT2_5mm_pulley_dia/35, h=100, center=true);
            translate([GT2_5mm_pulley_dia/2.5, 0, 0])
                 cylinder(r=GT2_5mm_pulley_dia/20, h=100, center=true);
                 }
                }
        }
     } 
...
//	translate ([0,0, pulley_b_ht + pulley_t_ht + retainer_ht ]) rotate ([0,180,0])
    ...
    angle = pulley_OD > pulley_b_dia ? 180 : 0;
    rotate(angle, [1, 0, 0])
	...
	rotate_extrude($fn=teeth*4)  
      ...
      if (draw_text) {
          ...
          translate([0, pulley_b_dia, -.1])
          mirror([0, 1, 0])
 linear_extrude(height = .5) {
    text(str(teeth), size = (pulley_b_dia-motor_shaft/2)/4, font=font, halign = "center", valign = "center", $fn = 16);
           }
  }
