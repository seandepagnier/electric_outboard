...
//thread_out(8,161);					// make an M8 x 16 ISO thread
...
thread_in_ring(.53*in,10,(23.5-.53*in)/2);
thread_in_pitch(.53*in,10,in/13);			// make an half inch thread with 13turns
...
module thread_in_ring(dia,hi,thk)
{
	difference()
	{
		...
		translate([0,0,-thk/2]) cylinder(r = (dia/2)+0.1, h = hi+thk);
	}
}
